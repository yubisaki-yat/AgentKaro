...
const BotControl: React.FC<BotControlProps> = ({ botId, title, icon: Icon, color, email, subscription, onLimitReached }) => {
   ...
   return (
      <div className="p-4 sm:p-6 lg:p-8 space-y-6 lg:space-y-10 max-w-[1600px] mx-auto pb-32">
         {/* Bot Hero Section */}
         <div className="relative p-6 glass-card overflow-hidden group border border-slate-200/50 dark:border-white/5 shadow-xl">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
               <div className="flex items-center gap-4">
                  <div className={`p-3.5 rounded-xl bg-[#1C4670] text-white shadow-xl shadow-[#1C4670]/20 border border-white/10`}>
                     <Icon className="w-5 h-5" />
                  </div>

                  <div>
                     <h1 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase mb-1">{title}</h1>
                     <div className="flex items-center gap-2">
                        <div className={`w-1.5 h-1.5 rounded-full ${running ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300 dark:bg-slate-700'}`} />
                        <span className="text-[9px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-500">
                           {running ? `Active: ${elapsed}` : 'Standby'}
                        </span>
                     </div>
                  </div>
         ...
         <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
            ...
            <div className="xl:col-span-1 space-y-6">
               <div className="bg-white dark:bg-slate-900/50 backdrop-blur-xl border border-slate-200/60 dark:border-slate-800/60 rounded-3xl p-6 shadow-xl shadow-slate-200/40 dark:shadow-none transition-all duration-300 hover:shadow-2xl hover:shadow-[#FFA229]/5">
                  <div className="flex items-center justify-between mb-6">
                     <div className="flex items-center gap-3">
                        <div className="p-2 bg-[#FFF2E5] dark:bg-[#1C4670]/20 rounded-xl">
                           <Settings className="w-4 h-4 text-[#1C4670] dark:text-[#FFA229]" />
                        </div>
                        <h3 className="text-lg font-black text-[#1C4670] dark:text-white tracking-tight uppercase">Config</h3>
                     </div>
            ...
            <div className="xl:col-span-2 flex flex-col h-[600px] bg-[#0a0f1c] rounded-3xl border border-slate-800 shadow-2xl overflow-hidden relative group">
               ...
               <div className="h-10 bg-slate-900/80 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-4 absolute top-0 w-full z-20">
                  ...
                  <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-2 opacity-50">
                     ...
                     <span className="text-[9px] font-mono font-bold text-white uppercase tracking-widest">Live_Stream</span>
                  </div>
                  <button
