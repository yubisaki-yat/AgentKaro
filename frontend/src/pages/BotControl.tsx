         ...
         <div className="relative p-8 glass-card overflow-hidden group border border-slate-200/50 dark:border-white/5 shadow-xl">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
               <div className="flex items-center gap-5">
                  <div className={`p-4 rounded-2xl bg-[#1C4670] text-white shadow-xl shadow-[#1C4670]/20 border border-white/10`}>
                     <Icon className="w-6 h-6" />
                  </div>
                  <div>
                     <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight uppercase mb-1">{title}</h1>
                     <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${running ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300 dark:bg-slate-700'}`} />
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-500">
                           {running ? `Active Session: ${elapsed}` : 'Engine Standby'}
                        </span>
                     </div>
                  </div>
               </div>
               ...
               <div className="flex items-center gap-4">
                  {!email ? (
                     <button
                        ...
                        className="px-8 py-3.5 bg-[#1C4670] text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-[#1C4670]/20 hover:scale-105 transition-all flex items-center gap-3"
                     >
                        <Play className="w-3.5 h-3.5" /> Sign In to Launch
                     </button>
                  ) : running ? (
                     <button
                        onClick={handleStop}
                        className="px-8 py-3.5 bg-rose-500 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-rose-500/20 hover:scale-105 transition-all flex items-center gap-3"
                     >
                        <Square className="w-3.5 h-3.5" /> Terminate Bot
                     </button>
                  ) : (
                     <button
                        onClick={handleStart}
                        disabled={botId === 'company_crawler' && subscription === 'free'}
                        className={`px-8 py-3.5 bg-[#FFA229] text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-[#FFA229]/20 hover:scale-105 transition-all flex items-center gap-3 ${botId === 'company_crawler' && subscription === 'free' ? 'opacity-50 grayscale cursor-not-allowed' : ''}`}
                     >
                        <Play className="w-3.5 h-3.5" /> {botId === 'company_crawler' && subscription === 'free' ? 'PRO Lifetime Only' : 'Launch Master Bot'}
                     </button>
                  )}
               </div>
            </div>
